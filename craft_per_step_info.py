...
def update_samples_log_weights_with_info(
    ...
    resample_threshold: float) -> Tuple[Array, Array, AcceptanceTuple, int]:
  ...
  return markov_samples, log_weights_resampled, acceptance_tuple, n_unique_samples
...
def get_craft_info_per_step(initial_sampler, flow_apply, markov_kernel_apply,
  ...
  for i in range(config.num_temps - 1):
    ...
    samples, log_weights, acceptance_tuple, n_unique_samples = update_samples_log_weights_with_info(
  ...
  info = {}
  info.update(n_unique_samples_hist=n_unique_samples_hist, log_weights=log_weights)
  return info
